...
instance (PGType t, PGParameter t (IdType a)) => PGParameter t (Id a) where
  pgEncode t (Id i) = pgEncode t i
  pgEncodeValue e t (Id i) = pgEncodeValue e t i
  pgLiteral t (Id i) = pgLiteral t i
instance (PGType t, PGColumn t (IdType a)) => PGColumn t (Id a) where
  pgDecode t = Id . pgDecode t
  pgDecodeValue e t = Id . pgDecodeValue e t
instance (PGType t, PGParameter t (IdType a), PGColumn t (IdType a), PGRep t (IdType a)) => PGRep t (Id a)
