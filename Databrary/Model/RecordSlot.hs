...
lookupVolumeContainersRecords v =
  map (second catMaybes) . groupTuplesBy ((==) `on` containerId) <$>
    dbQuery (($ v) <$> $(selectQuery selectVolumeSlotMaybeRecord "WHERE container.volume = ${volumeId v} AND NOT container.top ORDER BY container.id, record.category NULLS FIRST, slot_record.segment, slot_record.record"))
